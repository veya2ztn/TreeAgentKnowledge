...
base_url = "https://api.lingyiwanwu.com/v1";model    = "yi-lightning" 
...
import numpy as np
root = f"results/single_to_single/{model}"
logP = f"results/single_to_single/{model}/logs"
...
async def process_contents(tree, contents_split):
    for round, content_list in enumerate(tqdm(contents_split)):
        ...
        with open(f"{logP}/{model}.logs.{round}.json",'w') as f:
            json.dump(logs,f)
        tree.save_to_json(f"{root}/{model}.snap.{round}.json")
